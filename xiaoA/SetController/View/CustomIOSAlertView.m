...
{
    MyLog(@"Button Clicked! %d, %d", (int)buttonIndex, (int)[alertView tag]);
    [self close];
}
...
//    dialogContainer.layer.borderWidth = 1;
    ...
    dialogContainer.layer.shadowOpacity = 0.f;
...
- (void)addButtonsToView: (UIView *)container
{
    ...
    for (self.i=0; self.i<[buttonTitles count]; self.i++) {

        UIButton *closeButton = [UIButton buttonWithType:UIButtonTypeCustom];

        [closeButton setFrame:CGRectMake(self.i * buttonWidth, container.bounds.size.height - buttonHeight, buttonWidth, buttonHeight)];

        [closeButton addTarget:self action:@selector(customIOS7dialogButtonTouchUpInside:) forControlEvents:UIControlEventTouchUpInside];
        [closeButton setTag:self.i];

        [closeButton setTitle:[buttonTitles objectAtIndex:self.i] forState:UIControlStateNormal];
        ...
        if (iPhone6p) {
            [closeButton.titleLabel setFont:[UIFont boldSystemFontOfSize:21.0f]];
        }
